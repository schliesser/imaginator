#!/usr/bin/env bash
#
# Build the demo instance for the CI e2e job: native MariaDB creds + a localhost BASE_URL, then an
# ImageMagick GFX config so the *local* image processor (the v1 default — no imgproxy in CI) can
...
echo "› configuring ImageMagick for the local image processor"
mkdir -p config/system
cat > config/system/additional.php <<'PHP'
<?php
// CI: the demo runs the local image processor (no imgproxy), so enable ImageMagick.
