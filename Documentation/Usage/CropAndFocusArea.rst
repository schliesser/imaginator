...
fits the requested ratio inside the **crop area**, centred on the **focus area**.
The framing the editor chose is honoured and **no crop geometry is exposed in the
URL** — in ``local:async`` mode the URL keys only on the reference uid (``r…``).
