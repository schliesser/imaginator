/*!
 * sizes="auto" polyfill for browsers without native support (Safari/iOS as of 2026).
 *
 * Vendored verbatim from Shopify/autosizes v1.0.0 (MIT).
 * Source: https://github.com/Shopify/autosizes (src/autosizes.js)
 *
 * Imaginator emits a server-rendered width ladder with sizes="auto" on every lazy <img>; this
 * progressive-enhancement script supplies a concrete pixel `sizes` on browsers that do not yet
 * understand sizes="auto", so they pick a right-sized candidate instead of the 100vw fallback.
 * It is never load-bearing for sharpness (the ladder already is) and backs off on native support.
 *
 * Do not edit by hand — re-vendor from upstream to update.
 */
(function () {
